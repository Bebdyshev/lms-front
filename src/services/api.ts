...
class LMSApiClient {
  ...
  async getHeadTeacherManagedCourses(): Promise<Array<{
    ...
    try {
      const response = await this.api.get('/head-teacher/courses');
      return response.data;
    } catch (error) {
  ...
  async getHeadTeacherTeacherDetails(courseId: number, teacherId: number, days: number = 30): Promise<{
    teacher_id: number;
    teacher_name: string;
    email: string;
    avatar_url: string | null;
    ...
    try {
      const response = await this.api.get(`/head-teacher/course/${courseId}/teacher/${teacherId}/details`, {
        params: { days }
      });
      return response.data;
    } catch (error) {
  ...
  async getHeadTeacherTeacherFeedbacks(
    courseId: number,
    teacherId: number,
    skip: number = 0,
    limit: number = 20
  ...
  async getHeadTeacherTeacherAssignments(
    courseId: number,
    teacherId: number,
    skip: number = 0,
    limit: number = 20
