...
export default function Modal({ open, title, children, onClose, onSubmit, submitText = 'Save', cancelText = 'Cancel', onCancel }: ModalProps) {
  ...
  const content = (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-lg border border-gray-200 w-full max-w-lg p-6">
        <div className="text-lg font-semibold mb-4">{title}</div>
        <div className="space-y-4 max-h-[70vh] overflow-auto">{children}</div>
        <div className="mt-6 flex items-center justify-end gap-3">
          <button onClick={onCancel || onClose} className="px-4 py-2 rounded-lg bg-gray-100 hover:bg-gray-200">{cancelText}</button>
