...
export default function QuizzesPage() {
  ...
  useEffect(() => {
    fetchQuizzes().then(qs => setItems(qs.map(q => ({ ...q, attemptsLeft: getQuizAttemptsLeft(q.id) }))));
  }, []);
          ...
          <tbody>
            {items.map(q => (
              <tr key={q.id} className="border-t">
                ...
                <td className="px-4 py-3 text-gray-600">{Math.round(q.timeLimitSec/60)} min</td>
