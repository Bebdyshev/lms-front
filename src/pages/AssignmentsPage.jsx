          ...
          <tbody>
            {items.map(a => (
              <tr key={a.id} className="border-t">
                ...
                <td className="px-4 py-3 text-gray-600">{new Date(a.deadlineISO).toLocaleDateString()}</td>
                <td className="px-4 py-3 text-gray-600">
                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${a.status?.status === 'graded' ? 'bg-green-100 text-green-800' : a.status?.status === 'submitted' ? 'bg-blue-100 text-blue-800' : a.status?.status === 'resubmission-allowed' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-700'}`}>
                    {a.status?.status || 'not-submitted'}
                  </span>
                </td>
