        ...
        <div className="flex-1 overflow-y-auto">
          {threads.map(t => (
            ...
            >
              ...
              <div className="text-xs text-gray-500">Updated {new Date(t.lastMessageAt).toLocaleString()}</div>
            </button>
          ))}
        </div>
      </aside>
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50">
          {msgs.map(m => (
            <div key={m.id} className="max-w-[70%]">
              <div className={`px-3 py-2 rounded-xl text-sm ${m.senderId === (localStorage.getItem('sid')||'demo') ? 'bg-blue-600 text-white ml-auto' : 'bg-white border'}`}
